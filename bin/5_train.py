...
from eipl.data import MultimodalDatasetTactile
from eipl.utils import EarlyStopping, check_args, set_logdir, normalization, resize_img
import ipdb

# load own library
sys.path.append("./libs/")
from fullBPTT import fullBPTTtrainerTactile#学習用自作コード
...
parser.add_argument("--pt_tactile_loss", type=float, default=0.1)
...
model = TACTILESARNN(
    ...
    joint_dim=9,
    k_dim=args.k_dim,
    k_dim_tactile=args.k_dim_tactile,
...
loss_weights = [args.img_loss, args.joint_loss, args.pt_loss, args.tactile_loss, args.pt_tactile_loss]
trainer = fullBPTTtrainerTactile(model, optimizer, loss_weights=loss_weights, device=device)
...
with tqdm(range(args.epoch)) as pbar_epoch:
    for epoch in pbar_epoch:
        ...
        writer.add_scalar("Loss/train_img_tactile_loss", train_results['pt_tactile_loss'], epoch)
        writer.add_scalar("Loss/test_img_tactile_loss", test_results['pt_tactile_loss'], epoch)
        # early stop
